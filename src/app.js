...
(function () {
  ...
  // Dish photos come from src/artwork/dishes.js, generated by
  // tools/fetch-cuisines.js out of the same Wikipedia lead images, and arrive
  // finished the same way.
  function dishFor(entry) {
    return (entry && window.MASQ_DISHES ? window.MASQ_DISHES[entry] : null) || null;
  }

  // The photographers behind the photos, ready to print. Most of those pictures
  // are Creative Commons, which lets anyone use them on the condition that
  // whoever took them is named — so the Credits screen names them.
  //
  // Built once rather than per render, and empty rather than broken if the
  // artwork file failed to load. A picture in the public domain with no author
  // on record is credited to its licence alone, which is all it asks for.
  ...
  //
  // 'plate' is that same landscape frame for dish photographs, cropped about the
  // middle instead: food is photographed centred on the plate, and the high crop
  // that saves a penguin's head would cut the top off a bowl.
  const ART_SHAPES = {
  ...
  // The 18px variants are the same art at a smaller box — resized here rather
  // than kept as second copies, which drifted from their 20px twins.
  ...
  // solid bells at the tips.
  //
  // One flat silhouette, not an outlined cone per horn: outlines at 20px put
  // four thin lines through the same few pixels and the shape stopped reading.
  // The horns are broad leaf-shaped cones — fat where they leave the crown,
  // tapering to a point at the bell — splayed left, up and right so the three
  // stay tellable apart at any size.
  //
  // Everything is filled in one colour and the crown is drawn last, so the horn
  // bases tuck behind its toothed rim with no seams to line up.
  ...
  // Options: a lighting board's faders. A gear would have echoed the ⚙ in the
  // header, which opens a different screen.
  ...
  class App extends React.Component {
    state = {
      ...
      categories: ['Biomes', 'Cuisines', 'Locations', 'Movie/TV Show Genres', 'Music Genres'],
      wordCategories: ['Animals', 'Food', 'Movies/TV', 'Objects'],
      selCategories: ['Biomes', 'Cuisines', 'Locations', 'Movie/TV Show Genres', 'Music Genres'],
    ...
    renderVals() {
      ...
      const { biomeCatalog, cuisineCatalog, locationCatalog, fakeLocationRoleCatalog, fakeBiomeRoleCatalog, fakeCuisineRoleCatalog, movieTvCatalog, fakeMovieTvRoleCatalog, musicGenreCatalog, fakeMusicGenreRoleCatalog, wordOnlyCatalog } = window.MASQ_LOCATIONS_DATA;
      ...
      const apArt = apWordPoster || apRolePoster || apRoleAlbum || apRoleAnimal || apRoleDish;
      ...
      const apArtCompact = !!(apRolePoster || apRoleAlbum || apRoleAnimal || apRoleDish) && showWord;
      const apArtShape = ART_SHAPES[apRoleDish ? 'plate' : (apRoleAnimal ? 'photo' : (apRoleAlbum ? 'cover' : 'poster'))];
      ...
      return {
        ...
        apRole: apRoleShown,
        apRoleColor: apIsUndisguisedJester ? '#b3202f' : (isBiomeRound ? '#2e5bb0' : (isCuisineRound ? '#a85a2b' : (isHistoricalRound ? '#b5893c' : (isMovieTvRound ? '#2f8f7a' : (isMusicRound ? '#6b4ea8' : 'var(--m-accent)'))))),
        // Same reason as apWordSize: artwork leaves less room for a long title,
        // and "Artist (Song)" runs longer than a film title does. Cuisines carry
        // no artwork but still sit below the default, for the likes of
        // "Schwarzwalder Kirschtorte".
        apRoleSize: apIsUndisguisedJester ? '26px' : (apRolePoster ? '17px' : (apRoleAlbum ? '16px' : ((apRoleAnimal || apRoleDish) ? '18px' : (isBiomeRound ? '22px' : (isCuisineRound ? '20px' : ((isMusicRound || isMovieTvRound) ? '19px' : '23px')))))),
        apWord: apWordShown,
        apArt, apArtW, apArtH, apArtFocus: apArtShape.focus,
        apWordLabel: isCustomRound ? 'Word' : (isBiomeRound ? 'Biome' : (isCuisineRound ? 'Cuisine' : (isHistoricalRound ? 'Era' : (isMovieTvRound ? 'Genre' : (isMusicRound ? 'Genre' : (isFoodRound ? 'Food' : (isAnimalsRound ? 'Animal' : (isObjectsRound ? 'Object' : (isMoviesWordRound ? 'Movie / TV' : 'Location'))))))))),
    ...
    creditsModal(v) {
      ...
      return h('div', { style: css('background:var(--m-modal); border-radius:22px 22px 0 0; padding:20px 20px 36px; border-top:1px solid var(--m-border-strong); max-height:80vh; overflow-y:auto; animation:masq-slide-up .3s ease both;') },
        ...
        // its own so four hundred names can be there in full without burying
        // the three people who made the game — and kept in separate boxes
        // rather than one, because a name is easier to find in the list it
        // belongs to.
